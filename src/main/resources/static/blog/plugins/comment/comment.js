$('#commentSubmit').click(function () {
    ...
    var text = document.getElementById('t').textContent;
    if (isNull(blogId)) {
        swal("参数异常", {
            icon: "warning",
        });
    ...
    if (isNull(commentator) && text == '0') {
        swal("请输入你的称呼", {
            icon: "warning",
        });
        return;
    }
    if (isNull(email)  && text == '0') {
        swal("请输入你的邮箱", {
            icon: "warning",
        });
        return;
    }
    if (isNull(verifyCode)  && text == '0') {
        swal("请输入验证码", {
            icon: "warning",
        });
        return;
    }
    // if (!validCN_ENString2_100(commentator)  && text == '0') {
