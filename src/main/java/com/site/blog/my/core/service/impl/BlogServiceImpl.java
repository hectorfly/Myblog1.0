...
//使用用户ID查找博客
    @Override
    public PageResult getBlogsPageByID(PageQueryUtil pageUtil, Integer userId) {
        pageUtil.setUserId(userId);
        List<Blog> blogList = blogMapper.findBlogListByUserId(pageUtil);
    ...
    private List<BlogListVO> getBlogListVOsByBlogs(List<Blog> blogList) {
        ...
        if (!CollectionUtils.isEmpty(blogList)) {
            ...
            for (Blog blog : blogList) {
                AdminUser adminUser = adminUserMapper.selectByPrimaryKey(blog.getBlogOwner());
                BlogListVO blogListVO = new BlogListVO();
                BeanUtils.copyProperties(blog, blogListVO);
                blogListVO.setUserAvatar(adminUser.getAvatar());
                blogListVO.setUserNickName(adminUser.getNickName());
